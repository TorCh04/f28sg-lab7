...
public class Trie implements TrieADT {
	...
	private class TrieNode { // This is the class that represents a node in the trie
		...
		public int countAllWords() {
			...
			// Loops through all possible letters of the current node
			for (int i = 0; i < subnodes.length; i++) 
			{
				// Checks if the letter exists in the sub node
				if (subnodes[i] != null)
				{
					// Checks if it has reached the end of the word in the sub node
					if (subnodes[i].isValidEnd) 
					{
						// If it has reached the end increment numberOfWords
						numberOfWords++;
					}
					// Recursively calls itself to add all the words in the sub node to the total
