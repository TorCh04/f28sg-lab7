...
public class DLinkedList {
	...
	public void insertionSort() {
		...
		// Iterates through each node, starting from the second node
		for (Node current = headNode.nextNode; current != null; current = current.nextNode)
		{
			Node previous = current.prevNode;
			// Loops while there is a previous node and the current node value is less that the previous node's value
			while (previous != null && current.getValue() < previous.getValue()) 
			{
				// Swaps the current value and the previous value
				insertAfter(current, delete(previous));
